...
# Helper function for converting a score model to a flow model for gaussian paths
def score2flow(score_model: ContextUnet, path: GaussianProbabilityPath) -> Callable:
    ...
    """
    assert isinstance(path, GaussianProbabilityPath), (
        "Only GaussianProbabilityPath is supported"
    )
    alpha = path.alpha
    beta = path.beta
...
class ODE:
    ...
    def velocity_field(
        ...
        gfc_drift = (
            1 - self.guidance_value
        ) * unconditional_drift + self.guidance_value * conditional_drift
...
class SDE:
    ...
    def score(self, x: torch.Tensor, c: torch.Tensor, t: torch.Tensor):
        ...
        if c is not None:
            ...
            gfc_v_param = (
                1 - self.guidance_value
            ) * unconditional_v_param + self.guidance_value * conditional_v_param
    ...
    def velocity_field(
        ...
        if c is not None:
            ...
            conditional_vector_field, unconditional_vector_field = vparam_output.chunk(
                2, dim=0
            )
            v_out = (
                (1 - self.guidance_value) * unconditional_vector_field
                + self.guidance_value * conditional_vector_field
            )
...
class ODEIntegrator(Simulator):
    ...
    def simulate(self, x0: torch.Tensor, c: torch.Tensor):
        ...
        if self.num_save == 1:
            ret_idx = torch.tensor(
                [self.num_steps - 1], device=x.device, dtype=torch.long
            )
        else:
            ret_idx = torch.linspace(
                0,
                self.num_steps - 1,
                steps=self.num_save,
                device=x.device,
...
class EulerMaruyama(Simulator):
    ...
    def simulate(self, x0: torch.Tensor, c: torch.Tensor):
        ...
        for k in tqdm(range(self.num_steps - 1)):
            ...
            x = (
                x
                + self.sde.drift(x, c, t[..., None, None, None]) * h
                + self.sde.diffusion_coeff(x, t[..., None, None, None])
                * torch.randn_like(x)
                * torch.sqrt(h)
            )
...
# -----------------#
# MAIN SAMPLER FOR CLEANING MODEL
# ABOVE CLASSES MOSTLY FOR PREDICTION MODEL INFERENCE
class LikelihoodLangevin:
    ...
    def star_fullres(self, residual, t):
        ...
        c_zero = torch.zeros(
            1, 1, self.context_len, device=residual.device, dtype=residual.dtype
        )
        g_tile_chunks = []
        ...
        for start in range(0, tiles.shape[0], mb):
            ...
            cm_mb = torch.zeros(
                n, *c_zero.shape[1:], device=residual.device, dtype=residual.dtype
            )

            # Assumes model trained with v parametrization
    ...
    def likelihood_score(self, X, t):
        ...
        # return negative grad for light by chain rule
    ...
    def simulate(self, X0):
        ...
        Returns:
            X: (B, 1, 4096, 4096) final light estimate
        """
        X = X0.clone()
